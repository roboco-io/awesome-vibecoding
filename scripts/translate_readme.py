...
def translate_chunk(chunk, target_language, user_request, retry_count=0):
    """
    Translate a chunk of text using OpenAI API with fallback options and retry logic
    """
    # Set OpenAI API key
...
{chunk}"""
    ...
    for model_index, model in enumerate(models):
        try:
            ...
            print(f"Successfully translated with model: {model}")
            return translated_chunk
            
        except openai.RateLimitError as e:
            print(f"Rate limit exceeded for model {model}: {str(e)}")
            if model_index < len(models) - 1:
                print(f"Trying fallback model: {models[model_index + 1]}")
            else:
                # If we've tried all models and still have rate limit issues
                if retry_count < 3:
                    retry_delay = (retry_count + 1) * 10  # Exponential backoff
                    print(f"All models rate-limited. Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)
                    return translate_chunk(chunk, target_language, user_request, retry_count + 1)
                else:
                    print("Maximum retry attempts reached. Translation failed.")
                    raise
            ...
            if model_index < len(models) - 1:
                ...
                print("All models failed. Translation failed.")
                raise
...
{source_content}"""
            ...
            for model_index, model in enumerate(models):
                try:
                    print(f"Attempting full translation with model: {model}")
                    # Call OpenAI API
                    response = openai.chat.completions.create(
                        model=model,
                        messages=[
                    ...
                    if model_index < len(models) - 1:
                        ...
                        print("All models rate-limited. Translation failed.")
                        raise
                except Exception as e:
                    print(f"Error with model {model}: {str(e)}")
                    if model_index < len(models) - 1:
            ...
            if translated_content is None:
                raise Exception("Translation failed with all models")
            ...
            print(f"Error translating full document: {str(e)}")
            print("Checking if we can use existing translation...")
            
            # If we have an existing translation but no previous source content,
            # we'll create the cache file with the current source content
            if target_exists:
                print(f"Using existing translation for {target_language} and creating cache for future diffs.")
                save_source_content(source_content, target_language)
                return
            else:
                raise
    ...
    for chunk_text, start_line, end_line in diff_chunks:
        ...
        try:
            ...
            if translated_start_line is None or translated_end_line is None or translated_start_line > translated_end_line:
                ...
                # Try to translate the full document with fallback models
                models = ["gpt-4o-mini"]
                new_translated_content = None
...
{source_content}"""
                        ...
                        # Get the translated content
                        new_translated_content = response.choices[0].message.content
                        print(f"Successfully translated with model: {model}")
                        break
                        ...
                        if model_index < len(models) - 1:
                            ...
                            print("All models rate-limited. Keeping existing translation.")
                            # If all models fail, we'll keep the existing translation
                            new_translated_content = None
                    except Exception as e:
                        print(f"Error with model {model}: {str(e)}")
                        if model_index < len(models) - 1:
                            ...
                            print("All models failed. Keeping existing translation.")
                            # If all models fail, we'll keep the existing translation
                            new_translated_content = None
                
                if new_translated_content:
                    # Add language notice at the top
                    if target_language == "Korean":
                        notice = "# Awesome Vibe Coding (한국어)\n\n"
                    elif target_language == "Japanese":
                    ...
                    # Replace the first heading and any existing language selection links
                    pattern = r'^# .*?$\n\n\*Language:.*?\*\n\n'
                    if re.search(pattern, new_translated_content, re.MULTILINE | re.DOTALL):
                        # If language selection links already exist, just replace the heading
                        translated_content = re.sub(pattern, notice + "*Language: [English](README.md) | [한국어](README.ko.md) | [日本語](README.ja.md)*\n\n", new_translated_content, count=1, flags=re.MULTILINE | re.DOTALL)
                    else:
                        # If no language selection links exist, add them
                        pattern = r'^# .*?$'
                        translated_content = re.sub(pattern, notice + "*Language: [English](README.md) | [한국어](README.ko.md) | [日本語](README.ja.md)*\n\n", new_translated_content, count=1, flags=re.MULTILINE)
                else:
                    print(f"Warning: Could not translate full document. Keeping existing translation for {target_language}.")
            else:
            ...
            print(f"Skipping chunk and continuing with next one...")
            continue
