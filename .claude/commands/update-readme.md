...
Research and update each section of the README with new tools using multiple sources and search strategies.
...
For each category, run multiple targeted queries:
...
1. **Read current README.md section** - Know what's already listed
2. **Run ALL search queries** for that category
3. **Compile candidate list** - Extract tool names from all searches
4. **Verify each candidate:**
   - Has official website or GitHub repo
   - Is actively maintained (commits in last 3 months)
   - Has documentation
   - Has meaningful community adoption (stars, users)
5. **Add new entries** that meet criteria
6. **Update existing entries** if they have significant new features
7. **Flag discontinued tools** for human review (don't auto-remove)
...
1. Number of new tools added per category
2. List of new tools with brief justification
3. Any tools flagged for review
4. Any tools that were considered but not added (and why)
